...
class CalculateHourlyStats(beam.PTransform):
    ...
    def calculate_total_distance(self):
        return beam.Map(
            lambda bucket: dict(**bucket, meters_to_prev=calculate_total_distance(bucket["msgs"]))
        )

    def calculate_avg_speed_in_knots(self):
        return beam.Map(
            lambda bucket: dict(**bucket, avg_speed_knots=calculate_avg_speed_in_knots(bucket))
        )

    def calculate_is_slow(self):
        return beam.Map(
            lambda bucket: dict(**bucket, slow=calculate_is_slow(bucket, self.slow_threshold))
        )
